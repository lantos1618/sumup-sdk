...
class SumUp {
    ...
    async listCheckouts(checkout: ListCheckoutRequest): Promise<ListCheckoutResponse> {
        ...
        const url = this.apiBaseURL + queryURL + '?' + queryParameters.toString();
        const method = 'GET';
    ...
    async getCheckout(checkout: GetCheckoutRequest): Promise<Checkout> {
        const queryURL = "/checkouts";
        // const queryParameters = new URLSearchParams(checkout);
        const url = this.apiBaseURL + queryURL + '/' + checkout.checkout_reference;
        const method = 'GET';
        const response = await fetch(url, {
            method,
            headers: {
    ...
    async processCheckout(checkout: ProcessCheckoutRequest): Promise<ProcessCheckoutResponse | ProcessCheckoutNextStep> {
        ...
        const url = this.apiBaseURL + queryURL + '/' + checkout.token;
        const method = 'PUT';
